...
var WebEditorConfig = {
    ...
    //----------------------------------------------------
    filepath         : ".",
    imgpath          : "/webeditor/img/WebEditor",        //버튼 이미지경로
    ...
    //cut    : ["잘라내기","cut.gif"],
    //copy: ["복사","copy.gif"],paste: ["붙여넣기","paste.gif"],
    source  : ["소스보기","source.gif"],
    bar : ["구분선","bar.gif"]
    },
    //----------------------------------------------------
    //기본버튼템플릿 (all,simple)
...
// WebEditor
function WebEditor(id)
{
    if(typeof(document.execCommand)=="undefined") return;
    WebEditorUtil.init();
    //config설정
    this.cfg        = WebEditorConfig;
    this.cfg.preid  = WebEditorConfig.name+"_"+id;
    this.cfg.Btn    = WebEditorConfig.BtnTemplate["all"]; //기본 버튼 템플릿
    this.mode       = "wysiwyg";
    this.btn        = "";
    this.sel        = null;
    this.range      = null;
...
WebEditor.prototype.init = function()
{
    ...
    //this._iframe.onmouseover = WebEditorUtil.hideDiv;
    //전체 div
    ...
    if(WebEditorUtil.isIE) {this._divbtn.style.width = this.cfg.width;}
    //삽입
    ...
    var self=this;
    WebEditorUtil.addEvent(this._doc, "mousedown", WebEditorUtil.hideDiv);
    if(WebEditorUtil.isIE)
    {
        WebEditorUtil.addEvent(this._doc, "keydown", function(e) {
...
//WebEditor.setBtn
WebEditor.prototype.setBtn = function()
{
    ...
    {
        tmp = "";
        order = WebEditorUtil.trim(arr[i]);
...
//WebEditor.cmd
WebEditor.prototype.cmd = function(btn, order, value)
{
    var self = (this) ? this:WebEditorUtil._editor;
    var doc  = self._doc;
    if(self.mode=="text" && order!="source") {alert("'소스보기' 해제후 사용해 주세요!");    return;}
    self.focus();
    WebEditorUtil.hideDiv();
    self.btn = btn;
    ...
    switch(order)
    {
        case "create_order":    //사용자 추가명령
            self.setSelection();
            WebEditorUtil._editor = self;
            WebEditorUtil.order = order;
            create_func(self);
        break;
        case "hyperlink":
            var link_text = (self.sel_html) ? self.sel_html : WebEditorUtil._linktxt.value;
            var html      = "<a href='"+WebEditorUtil._linktxt.value+"' target='_blank'>"+link_text+"</a>";
            self.innerHTML(html);
        break;
        ...
        case "link" :
            var div=null;
            if(order=="color")        { order = "forecolor"; WebEditorUtil.tblSet_color(); div=WebEditorUtil._colortbl;}
            else if(order=="hilite")  { order = "hilitecolor"; WebEditorUtil.tblSet_color(); div=WebEditorUtil._colortbl;}
            else if(order=="font")    { order = "fontname"; WebEditorUtil.tblSet_font(); div=WebEditorUtil._fonttbl;}
            else if(order=="size")    { order = "fontsize"; WebEditorUtil.tblSet_size(); div=WebEditorUtil._sizetbl;}
            else if(order=="link")
            {
                order = "hyperlink";
                WebEditorUtil.tblSet_link();
                div=WebEditorUtil._linktbl;
                WebEditorUtil._linktxt.value = "http://";
                self.setSelection();
            }

            WebEditorUtil._editor = self;
            WebEditorUtil.order = order;
            WebEditorUtil.showDiv(div);
        break;
        case "source":
        ...
        default:
            ...
            else if(order=="hilitecolor" && WebEditorUtil.isIE) order = "backcolor";
            doc.execCommand(order, false, value);
        break;
    }
};
//WebEditor.focus
WebEditor.prototype.focus = function()
{
...
//WebEditor.setSelection
WebEditor.prototype.setSelection = function()
{
...
//WebEditor.innerHTML
WebEditor.prototype.innerHTML = function(html)
{
    if(WebEditorUtil.mode=="text") return;
...
var WebEditorUtil = {
    ...
    init : function()
    {
        if(WebEditorUtil.is_init) return;

        var s="<style>";
            s += ".easyWebEditorDiv a {text-decoration:none;color:#000}"
            s += "#"+WebEditorConfig.name+"_colortbl a {border:1px solid #f5f5f5;padding:0 6px;height:9px;font:8px verdana;text-decoration:none}";
            s += "</style>";
        document.write(s);
        WebEditorUtil.is_init=1;
    },
    showDiv : function(div)
    {
        var btn           = WebEditorUtil._editor.btn;
        div.style.top     = WebEditorUtil.curTop(btn) + btn.offsetHeight + "px";
        div.style.left    = WebEditorUtil.curLeft(btn) + "px";
        div.style.display = "";
    },
    hideDiv : function()
    {
        if(typeof(WebEditorUtil)!="object") return;
        for(var i=0; i<WebEditorUtil.arrtbl.length; i++)
        {
            try{ eval('WebEditorUtil._'+WebEditorUtil.arrtbl[i]+'tbl.style.display="none"'); }
            catch(e) { }
        }
    },
    curTop : function(el)
    ...
    {
        if(WebEditorUtil._linktbl) return;
        var id = WebEditorConfig.name+"_linktxt";
        var s = "<input type='text' value='http://' style='width:230px;font:8pt 돋움;color:gray' id='"+id+"' /><br>";
            s += "링크주소(URL)를 넣어주세요<br><br>";
            s += "<a href='javascript:;' onclick=\"WebEditorUtil._editor.cmd(null, WebEditorUtil.order, '');\">확인</a>";

        var div = WebEditorUtil.getDiv(WebEditorConfig.name+"_linktbl",s);
        div.style.padding = "15px";
        div.style.font = "8pt 돋움";
        document.body.appendChild(div);
        WebEditorUtil._linktbl = div;
        WebEditorUtil._linktxt = document.getElementById(id);
    },
    tblSet_size : function()
    {
        if(WebEditorUtil._sizetbl) return;
        var size = new Array(8,10,12,14,18,24);
        var s    = "";

        for(var i=0; i<size.length; i++)
        {
            s += "<a href='javascript:;' onclick=\"WebEditorUtil._editor.cmd(null, WebEditorUtil.order,'"+(i+1)+"');\" style='font:"+size[i]+"pt 굴림;'>가나다라 ("+size[i]+")</a><br />";
        }
        var div = WebEditorUtil.getDiv(WebEditorConfig.name+"_sizetbl",s);
        div.style.padding = "5px";
        document.body.appendChild(div);
        WebEditorUtil._sizetbl = div;
    },
    tblSet_font : function()
    {
        if(WebEditorUtil._fonttbl) return;
        ...
        for(var i=0; i<font.length; i++)
        {
            ...
            s += "<a href='javascript:;' onclick=\"WebEditorUtil._editor.cmd(null, WebEditorUtil.order,'"+font[i]+"');\" style='font:10pt "+font[i]+";line-height:170%'>"+txt+" ("+font[i]+")</a><br />";
        }
        var div = WebEditorUtil.getDiv(WebEditorConfig.name+"_fonttbl",s);
        div.style.padding = "5px";
        document.body.appendChild(div);
        WebEditorUtil._fonttbl = div;
    },
    tblSet_color : function()
    {
        if(WebEditorUtil._colortbl) return;
        var col= new Array();
        ...
        for(var i=0; i<10; i++)
        {
            ...
            {
                color = col[i][j];
                s += "<a href='javascript:;' onclick=\"WebEditorUtil._editor.cmd(null, WebEditorUtil.order,'"+color+"');\" style='background-color:"+color+";'>&nbsp;</a>";
            }
            s += "<br />";
        }
        var div = WebEditorUtil.getDiv(WebEditorConfig.name+"_colortbl",s);
        document.body.appendChild(div);
        WebEditorUtil._colortbl = div;
    },
...
//--------------------------------------------------------
//명령(버튼) 추가
WebEditorConfig.BtnList.table = ["테이블 삽입","table.gif",
function (self){
    window.open("/webeditor/inc/web_table.jsp","table","width=400,height=240,status=1");
} ];
...
WebEditorConfig.BtnList.preview = ["미리보기","preview.gif",
function (self){
...
//업로드이미지버튼 정의///////////////////////////////////////////////////////////////////////////////////
WebEditorConfig.BtnList.upload_image = ["이미지 업로드","image.gif",
function (self){
 window.open("/webeditor/inc/upload_image.jsp","image","width=300,height=390,status=1");
}];
//업로드이미지버튼 정의///////////////////////////////////////////////////////////////////////////////////

WebEditorConfig.BtnList.image = ["이미지 삽입","image.gif",
function (self){
    var order = "image";
    var txt_id = WebEditorConfig.name+"_imagetxt";
    var div_id = WebEditorConfig.name+"_imagetbl";
    var div = null;
    ...
    if(self.btn) //image 레이어 보여주기
    {
        if(!document.getElementById(div_id))
        {
            var s="";
                ...
                s += "<a href='javascript:;' onclick=\"WebEditorUtil._editor.cmd(null, '"+order+"', '');\">확인</a>";
            div = WebEditorUtil.getDiv(div_id,s);
            div.style.padding = "15px";
            div.style.font = "8pt 돋움";
            document.body.appendChild(div);
            WebEditorUtil._imagetbl = div;
            WebEditorUtil.arrtbl.push(order);
        }
        else
        {

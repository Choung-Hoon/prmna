...
<%
	...
	// 업종이 한건이지만 나머지 필드에 <br>로 되어 있으면 <tr>사이즈를 없앤다
	String trSize = "height=27 style='height:20.25pt'" ;
		
	if ( f2Str.length == 1) {
		// first_year
		fstYrStr = new String[]{excelData.getFstYr()};
		if (excelData.getFstYr().indexOf("<br>") > 0) trSize = "" ;
		// second_year
		scndYrStr = new String[]{excelData.getScndYr()};
		if (excelData.getScndYr().indexOf("<br>") > 0) trSize = "" ;
		// 2009
		f10Str = new String[]{excelData.getF10()};
		if (excelData.getF10().indexOf("<br>") > 0) trSize = "" ;
		// 2010
		f11Str = new String[]{excelData.getF11()};
		if (excelData.getF11().indexOf("<br>") > 0) trSize = "" ;
		// 2011
		f12Str = new String[]{excelData.getF12()};
		if (excelData.getF12().indexOf("<br>") > 0) trSize = "" ;
		// 3년합계
		f13Str = new String[]{excelData.getF13()};
		if (excelData.getF13().indexOf("<br>") > 0) trSize = "" ;
		// 5년합계
		sumOf5yrStr = new String[]{excelData.getSumOf5yr()};
		if (excelData.getSumOf5yr().indexOf("<br>") > 0) trSize = "" ;
		// 2012
		f14Str = new String[]{excelData.getF14()};
		if (excelData.getF14().indexOf("<br>") > 0) trSize = "" ;
		// 2008 ==> 2013변경
		f9Str = new String[]{excelData.getF9()};
		if (excelData.getF9().indexOf("<br>") > 0) trSize = "" ;

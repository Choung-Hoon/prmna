...
function zoomInOut(contentid, how)
{
  ...
  if (how == "in") {
  	if(bIE) {
    	...
    	document.body.style.zoom = curRate + '%';	/* ??? ??? */s
    }
    else {
    	scaleFont(+1);
    }
...
function menuclick(faq){
    if( showmenu != faq ) {//showmenu?? ??? id ??????? ?????? ????y? ??? privacy?? ??
  if( showmenu !="" ) {//showmenu?? ????? ???? showmenu?? ?????? id?? display="none"
   eval(document.getElementById(showmenu)).style.display = "none";
  }
  eval(document.getElementById(faq)).style.display = "block"; //????? privacy?? display="block"
  showmenu = faq;//????? ????? submenu id?? ??????
    } else {
...
//open 기본 popup
function l_viewPopup(url, width, height){
    var left = (window.screen.availWidth - width)/2 ;
    var top  = (window.screen.availHeight - height)/2 ;
    var obj  = window.dialogArguments;
    if(obj){
        dialogArguments.window.open(url,"popup","scrollbars=no, status=yes, width="+width+"px, height="+height+"px,top="+top+"px,left="+left+"px;");
    }else{
        window.open(url,"popup","scrollbars=no, status=yes, width="+width+"px, height="+height+"px,top="+top+"px,left="+left+"px;");
    }
}

//window.open Popup
/*************************************************************
*    l_viewPopup으로 열린 페이지에서 독립팝업 새창으로 오픈시
*    url     : 주소
*    title   : 팝업창 이름 (주의 : l_viewPopup 창의 이름(popup)과 중복시 target="_self")
*    width   : 팝업창 너비
*    height  : 팝업창 높이
**************************************************************/
function l_viewPopup2(url,title ,width, height)
//2010.03.15, test 서버에서는 status를 추가함
{
    var left = (window.screen.availWidth - width)/2 ;
    var top  = (window.screen.availHeight - height)/2 ;
    var obj  = window.dialogArguments;
    if(obj){
        openwin = dialogArguments.window.open(url,title,"scrollbars=yes, status=no , width="+width+"px, height="+height+"px,top="+top+"px,left="+left+"px;");
        openwin.focus();
    }else{
        openwin = window.open(url,title,"scrollbars=yes, status=no, width="+width+"px, height="+height+"px,top="+top+"px,left="+left+"px;");
        openwin.focus();
    }
}
